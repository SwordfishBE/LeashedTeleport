...
public final class ModrinthUpdateChecker {
    ...
    private static void checkForUpdate() {
        HttpRequest request = HttpRequest.newBuilder()
                ...
                .header("User-Agent", "LeashedTeleport/" + LeashedTeleportMod.VERSION)
                .GET()
                .build();
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                LeashedTeleportMod.LOGGER.debug("[LeashedTeleport] Update check returned HTTP {}.", response.statusCode());
                return;
            }

            Optional<String> latestVersion = extractLatestVersion(response.body());
            if (latestVersion.isEmpty()) {
                LeashedTeleportMod.LOGGER.debug("[LeashedTeleport] Update check returned no usable versions.");
                return;
            }

            String newestVersion = latestVersion.get();
            if (isNewerVersion(newestVersion, LeashedTeleportMod.VERSION)) {
                LeashedTeleportMod.LOGGER.debug("[LeashedTeleport] New version available: {} (current: {})",
                        newestVersion, LeashedTeleportMod.VERSION);
            }
        } catch (IOException | InterruptedException e) {
            if (e instanceof InterruptedException) {
                Thread.currentThread().interrupt();
            }
            LeashedTeleportMod.LOGGER.debug("[LeashedTeleport] Update check failed.", e);
        }
    }
